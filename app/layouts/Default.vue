<template>
  <div id="MainLayout" class="w-full fixed z-50">
          ...
          >
            ...
            <ul class="bg-white">
              <li
                ...
                class="py-2 px-4 w-full text-[13px] hover:bg-gray-200"
              >
                My Orders
              </li>
              <li
    ...
    <div id="MainHeader" class="flex items-center w-full bg-white">
      ...
      >
        ...
        <div class="w-full max-w-[700px] md:block hidden">
          <div class="relative">
            <div class="flex items-center border-2 rounded-md w-full border-[#FF4646]">
              <input
                type="text"
              ...
              <button class="flex items-center h-[100%] p-1.5 px-2 bg-[#FF4446]">
                <Icon name="ph:magnifying-glass" size="20" color="#ffffff" />
              </button>
            </div>

            <div class="absolute bg-white h-auto w-full max-w-[700px]">
              <div class="p-1" v-if="items && items.data" v-for="item in items.data" :key="item.id">
                <NuxtLink
                  :to="`/item/${item.id}`"
                ...
                >
                  <div class="flex items-center">
                    ...
                    <div class="trucate ml-2">{{ item.title }}</div>
                  </div>
                  <div class="trucate">$ {{ item.price / 100 }}</div>
                </NuxtLink>
              </div>
            </div>
          </div>
  ...
  <div class="lg:pt-[130px] md:pt-[130px] pt-[80px]"></div>
  <slot />
...
<script setup>
  ...
  const searchByName = useDebounce(async () => {
    ...
    if (searchItem.value) {
      items.value = await useLazyFetch(`/api/prisma/search-by-name/${searchItem.value}`);
    }
